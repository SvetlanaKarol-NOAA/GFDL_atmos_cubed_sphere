...
! </table>
      ...
      integer :: md_tadj_layers
      real tau_visc, tau_cond, tau_diff
      real md_consv_te
      real md_wait_hr
      real md_wait_sec
      
      logical :: md_time, md_eul, mda_pt, mda_uv, mda_wel, mda_trc
      logical :: mdz_uv, mdz_tk,  mdz_w,   mdz_q4
      integer :: md_lim16
      
      real, parameter:: Runiv = 8314.              
...
! --------------------------------------------------------
      ...
      namelist /molecular_diffusion_nml/ tau_visc, tau_cond, tau_diff, &
             ...
             md_wait_hr, md_consv_te, md_eul, mda_pt, mda_uv, mda_wel, mda_trc, &
	     mdz_uv, mdz_tk,  mdz_w,  mdz_q4, md_lim16
      ...
      md_lim16 = 1
!
! vertical molecular diffusion , implicit
!  
       mdz_tk = .true.  ; mdz_q4 = .true.
       mdz_uv = .false. ;  mdz_w = .false.     
      unit = stdlog()
...
!      if( spo2.gt.ind_gas_end ) spo2=0
      ...
      do n=1,dim
!check
!        if(plyr(n).le.0.0) call mpp_error(FATAL,"ERROR MDIF non positive value of plyr")
!        if(temp(n).le.0.0) call mpp_error(FATAL,"ERROR MDIF non positive value of temp")
...
!        endif
        ...
        if(spo .ne.0) qo   = max(0.0,q(n,spo ))
        if(spo2.ne.0) qo2  = max(0.0,q(n,spo2))
        if(spo3.ne.0) qo3  = max(0.0,q(n,spo3))
...
!
! VAY-2023: subroutine thermosphere_adjustment  "should" be dropped off (not used)
!      
