...
! Constant pressure                                phys_hydrostatic = .false. DONOT WORK !!!!! 
             do j=js,je
                call moist_cp(is,ie,isd,ied,jsd,jed, npz, j, k, nwat, sphum, liq_wat, rainwat,    &
                              ice_wat, snowwat, graupel, hailwat, q, qc, cvm, pt(is:ie,j,k) )
                do i=is,ie
...
!================================================================ THIS  IS PHYS_UPDATE !!!!!	    
               do j=js,je
                  call moist_cv(is,ie,isd,ied,jsd,jed, npz, j, k, nwat, sphum, liq_wat, rainwat,    &
                                ice_wat, snowwat, graupel, hailwat, q, qc, cvm, pt(is:ie,j,k))
                  do i=is,ie
!!! WRONG                     pt(i,j,k) = pt(i,j,k) + t_dt(i,j,k)*dt*con_cp/cvm(i)
!!!! CORRECTION-JAN-2024
                     pt(i,j,k) = pt(i,j,k) + t_dt(i,j,k)*dt  
		     
!!!! VAY CUA/NASA CORRECTION-JAN-2024	
