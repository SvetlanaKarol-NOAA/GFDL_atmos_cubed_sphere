...
!------------------
   ...
   real, intent(in):: damp(bd%isd:bd%ied, bd%jsd:bd%jed)
   real, intent(inout):: q(bd%isd:bd%ied, bd%jsd:bd%jed)     ! q ghosted on input
   real, intent(inout):: wdis(bd%isd:bd%ied, bd%jsd:bd%jed)  ! wdis = Kdis*dt*W*(Wx+Wy)   
   type(fv_grid_type), intent(IN), target :: gridstruct
! local:
   real fx2(bd%isd:bd%ied,bd%jsd:bd%jed), fy2(bd%isd:bd%ied,bd%jsd:bd%jed)
   real xv(bd%isd:bd%ied,bd%jsd:bd%jed)
   real fx2d(bd%isd:bd%ied,bd%jsd:bd%jed), fy2d(bd%isd:bd%ied,bd%jsd:bd%jed)   
   real d2(bd%isd:bd%ied,bd%jsd:bd%jed)
   integer i, j, m, n, nt, i1, i2, j1, j2
   real :: wold, fdisdt, fvdis
#ifdef USE_SG
   real, pointer, dimension(:,:)   :: dx, dy, rdxc, rdyc
   real, pointer, dimension(:,:,:) :: sin_sg
...
!
   do j=j1,j2
      do i=i1+1,i2
         ...
         fx2(i,j) = 0.5*(xv(i-1,j)+xv(i,j))*fx2(i,j)
	 fx2d(i,j) = fx2(i,j)*0.5*(d2(i-1,j)+d2(i,j))
      enddo
   enddo
! for dady * dx
  if( nord>0 ) then
...
!
   do i=i1,i2
      do j=j1+1,j2
         ...
         fy2(i,j) = 0.5*(xv(i,j-1)+xv(i,j))*fy2(i,j)
	 fy2d(i,j) = fy2(i,j)*0.5*(d2(i,j-1)+d2(i,j)) 
      enddo
   enddo
!wold, fvdis fdisdt
   do j=j1+1,j2-1
      do i=i1+1,i2-1
         ...
         (fx2(i,j)-fx2(i+1,j)+fy2(i,j)-fy2(i,j+1))*gridstruct%rarea(i,j)
!
! -d2(i,j)*0.5*(xv(i,j-1)+xv(i,j))*del6_uj +(xv(i,j)+xv(i,jp))*del6_ujp)
! -d2(i,j)*0.5*(xv(i-1,j)+xv(i,j))*del6_vi +(xv(i,j)+xv(ip,j))*del6_vip)
!  +d2(i,j) coeff=(1.-rarea_ij*[kaver *gfac] gfac = sinu_v*dy/dxc ...sin_v*dx/dyc
!           > stability coeff ~ [4*K*dt/dS]
! FV-dissipative heat
! Ptot = P1+P2_dis = W x dW/dt  
! P1 = d/dx[KwdW/dx]+ d/dy[KwdW/dy]  Kw = K*W
! P2_dis = Ptot -P1
!	 
	 fdisdt = .5*(wold+q(i,j))*(q(i,j)-wold)
         fvdis = fdisdt-(fx2d(i,j)-fx2d(i+1,j)+fy2d(i,j)-fy2d(i,j+1))*gridstruct%rarea(i,j)
	 wdis(i,j) = max(0.0, -fvdis)
      enddo
   enddo
...
!------------------
   ...
   real, intent(inout):: udis(bd%isd:bd%ied  , bd%jsd:bd%jed+1)
   real, intent(inout):: vdis(bd%isd:bd%ied+1, bd%jsd:bd%jed  )   
...
! local:
   ...
   real dfx1(bd%isd:bd%ied,bd%jsd:bd%jed), dfy1(bd%isd:bd%ied,bd%jsd:bd%jed)
   real dfx2(bd%isd:bd%ied,bd%jsd:bd%jed), dfy2(bd%isd:bd%ied,bd%jsd:bd%jed)
   ...
   real d2(bd%isd:bd%ied+1,bd%jsd:bd%jed  )
   integer i, j, m, n, nt, i1, i2, j1, j2
   real :: ufdisdt, vfdisdt, ufvdis, vfvdis
   real :: uold, vold   
#ifdef USE_SG
   real, pointer, dimension(:,:)   ::  dxa,  dya,  dx6,  dy6
   real, pointer, dimension(:,:)   :: rdxa, rdya, rdx6, rdy6
...
! dux
   do j=j1+1,j2
      do i=i1+1,i2
         ...
         fx1(i,j) = x6(i,j)*fx1(i,j)
	 dfx1(i,j)= fx1(i,j)*0.5*(d1(i-1,j)+d1(i,j))
      enddo
   enddo
! dvx
   do j=j1,j2
      do i=i1,i2
         ...
         fx2(i,j) = x5(i,j)*fx2(i,j)
	 dfx2(i,j) = fx2(i,j)*0.5*(d2(i,j)+d2(i+1,j))
      enddo
   enddo
!
  if( nord>0 ) then
...
! duy
   do j=j1,j2
      do i=i1,i2
         ...
         fy1(i,j) = x5(i,j)*fy1(i,j)
	 dfy1(i,j) = fy1(i,j) *0.5*(d1(i,j)+d1(i,j+1))
      enddo
   enddo
! dvy
   do j=j1+1,j2
      do i=i1+1,i2
         ...
         fy2(i,j) = x6(i,j)*fy2(i,j)
         dfy2(i,j) = fy2(i,j) * 0.5*(d2(i,j-1)+d2(i,j))	 
      enddo
   enddo
! ------------------
   do j=j1+1,j2
	 ...
	 ufdisdt = .5*(uold+u(i,j))*(u(i,j)-uold)
         ufvdis = ufdisdt-(dfx1(i,j)-dfx1(i+1,j)+dfy1(i,j-1)-dfy1(i,j))*gridstruct%rarea_u(i,j)
	 udis(i,j) = max(0.0, -ufvdis)		  
!ufdisdt, vdisdt, ufvdis, vfvdis
      enddo
   enddo
   do j=j1+1,j2-1
      do i=i1+1,i2
	 ...
	 vfdisdt = .5*(vold+v(i,j))*(v(i,j)-vold)
         vfvdis = vfdisdt-(dfx2(i-1,j)-dfx2(i,j)+dfy2(i,j)-dfy2(i,j+1))*gridstruct%rarea_v(i,j)	 	  
         vdis(i,j) =  max(0.0, -vfvdis)
